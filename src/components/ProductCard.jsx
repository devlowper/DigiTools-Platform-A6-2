...
const ProductCard = ({ product, carts, setCarts }) => {
  ...
  return (
    <div className="rounded-[16px] p-6 flex flex-col gap-4 border-2 border-[#f2f2f2] bg-white relative hover:shadow-lg transition-shadow">
      {/* Tag Badge */}
      <span
      ...
      {/* Icon */}
      <div className="w-[60px] h-[60px] rounded-full border border-[#f2f2f2] flex items-center justify-center text-3xl bg-[#f9fafc]">
        {icon}
      </div>

      {/* Name */}
      <h3 className="text-2xl font-bold text-[#101727] pr-24">{name}</h3>
      ...
      <ul className="flex flex-col gap-2">
        {features.map((feature) => (
          <li key={feature} className="flex items-center gap-2 text-base text-[#627382] font-medium">
            <span className="text-[#0a883e] font-bold">✓</span>
            {feature}
          </li>
        ))}
      </ul>
      ...
      <button
        ...
        className={`rounded-full h-[52px] w-full font-bold mt-auto transition-all cursor-pointer ${
          isInCart
